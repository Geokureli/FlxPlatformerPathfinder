...
import flixel.tile.FlxPlatformerPathfinder;
...
class PlayState extends FlxState
{
	...
	var _status:FlxText;
	var _coins:FlxGroup;
	var _pathfinder:FlxPlatformerPathfinder;
	...
	override public function create():Void
	{
		...
		_pathfinder = FlxPlatformerPathfinder.fromObject(_player, _player.maxVelocity.y / 2, FlxPoint.weak(8, 8));
	...
	override public function update(elapsed:Float):Void
	{
		...
		if (FlxG.mouse.justPressed)
		{
			trace("pressed");
			_player.path.cancel();
			var nodes = _pathfinder.findPathFromJumperToMouse(_level, _player);
			if (nodes != null)
				_player.path.start(nodes, 50);
		}
		
		if (_player.path.finished)
		{
			_player.path.cancel();
		}
	}
	...
	{
		super.draw();
		if (_player.path.finished == false)
			_player.path.drawDebug();
	}
